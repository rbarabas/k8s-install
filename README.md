...
## Check install pre-requisities such as the presence of enough memory and swap disabled:

`$ ansible-playbook plays/check.yml`

## Ad-hoc playbook to disable swap in both /etc/fstab and active swap configuration:

`$ ansible-playbook plays/prepare.yml`

## Connect tailscale

```
tailscale up --accept-routes --ssh --advertise-tags="tag:ssh-prod"
```

## Initial setup of the K8s cluster using Cilium as the CNI:
